...
export class DiagramComponent {
  ...
  public showDiagram(elements: DiagramElement[], bSteps: Array<Step>) {
    ...
    this.saveGraphState();
    if (bSteps.length > 3) {
      for (let i = 1; i <= (bSteps.length - 3); i++) {
        this.zoomOut();
        this.zoomOut();
        this.zoomOut();
      }
  ...
  get argSystem(): IArgSystem {
    return this._argSystem;
  }

  set argSystem(value: IArgSystem) {
    this._argSystem = value;
  }
}
