...
import {Actor, DiagramElement, Limitation, Rationale, Util} from '../../business/diagram/diagram';
...
export class PropertiesComponent implements OnChanges {
    ...
    private  loadVisualSettings() {
        ...
        for (let i = 0; i < this.selectedElement.artifacts.length; i++) {
            const artifact = this.selectedElement.artifacts[i];
            ...
            if (artifact instanceof Actor) {
                ...
                if (artifact.jsonElement.axonicProject) {
                    for (const r of Object.values(artifact.jsonElement.axonicProject)) {
    ...
    private  updateVisualSettings() {
        const lastwidth = this.selectedElement.visualShape.attributes.size.width;
        if (this.selectedElement.visualShape.attributes.attrs){
            ...
            if ((this.selectedElement.visualShape as any).portData.ports[1]){
                for (let i = 1; i < (this.selectedElement.visualShape as any).portData.ports.length; i++) {
        ...
        if (this.limitExist) {
            //Set visual properties of Limits
            for (const port of (this.selectedElement.visualShape as any).portData.ports){
                if (port.attrs.rect) {
    ...
    private addBusinessGroupsWithElements(){
        const properties = [];
        const keys = this.createKeysFromJson(this.selectedElement.jsonElement[0], '');
        ...
        keys.sort((a, b) => (a.key > b.key) ? 1 : ((b.key > a.key) ? -1 : 0))
            .forEach( arrayItem => {
            ...
            const itemViewKey = arrayItem.key.replace(/\[\w*\]/g, '')
                .replace(/\._/g, '');
